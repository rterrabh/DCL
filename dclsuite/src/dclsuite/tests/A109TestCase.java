 ...
 */
public class A109TestCase extends DCLTestCase { 

	public void test01() throws Exception {
		List<ArchitecturalDrift> violations = this.validateSystem("only com.example.c.C109 can-declare com.example.b.B109"); //Define the constraint to be validated
		...
		assertEquals(DeclareParameterDependency.class, dad.getForbiddenDependency().getClass()); //Check the type of dependency
		DeclareParameterDependency declareParameterDependency = (DeclareParameterDependency) dad.getForbiddenDependency();

		
		//Check each attribute of the violation
		assertEquals("com.example.a.A109",declareParameterDependency.getClassNameA());
		assertEquals("com.example.b.B109",declareParameterDependency.getClassNameB());
		
		assertEquals("f",declareParameterDependency.getMethodName());
	}
