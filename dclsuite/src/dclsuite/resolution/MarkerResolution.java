...
public class MarkerResolution implements IMarkerResolutionGenerator, IMarkerResolutionGenerator2 {
	...
	private static final IMarkerResolution noResolutionMarker = FixingUtil.createMarkerResolution("None (" + '\u2205'
	...
	public IMarkerResolution[] getResolutions(IMarker marker) {
		...
		try {
			...
			if (violationType.equals(ArchitecturalDrift.DIVERGENCE)) {
				...
				suggestions = DivergenceResolution.getSuggestions(marker, architecture, dependency, moduleDescriptionA,
						moduleDescriptionB, violatedConstraint);
			} else if (violationType.equals(ArchitecturalDrift.ABSENCE)) {
				MissingDependency missingDependency = new MissingDependency(moduleDescriptionA, classNameA,
						moduleDescriptionB, dependencyType);
