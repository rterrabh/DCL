...
public class DCLParser {
	...
	/**
	 * DCL2 Method responsible to parse the DCL specifications file and 
	 * to return a list of modules
	 * 
	 * @param in InputStream of the DCL specifications file
	 * @throws IOException Error on read the file
	...
	/**
	 * DCL2 Method responsible to parse the DCL specifications file and 
	 * to return a list of dependency constraints
	 * 
	 * @param in InputStream of the DCL specifications file
	 * @throws IOException Error on read the file
	 */
	public static final Collection<DependencyConstraint> parseDependencyConstraints(final IProject project, final InputStream in) throws IOException, CoreException {
		...
		while (lnr.ready()) {
			...
			if (line.startsWith("%")) {
				...
				int index = (line.indexOf("can") != -1) ? line.indexOf("can") : line
						.indexOf("must");
				...
				for (String restriction : restrictions.split(",")) {
					...
					dependencyConstraints.add(
							new DependencyConstraint(moduleDescriptionA, moduleDescriptionB, c));
				}
			}
		}
